...
这里严格递增/减和非严格递增/减需要按照题目分析，一般而言都是 `st.top() >=/<= nums[i]`，也就是严格，但是也有例外，如 [LC.84](https://leetcode.cn/problems/largest-rectangle-in-histogram/) 的出栈条件就是 `st.top() > price`，也就是栈剩余元素都是非严格递增的（按照题意，得到的**非严格递增栈**所有元素都要出栈操作，最终栈为空~）
