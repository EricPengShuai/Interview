...
> 2023.10.12 11:00-12:00 推荐架构的部署

- muduo 网络库具体说一下，学到什么东西

- 其中的 WebServer 在处理请求的怎么解析的，如何实现连接复用

  > 所谓复用就是一个连接处理很多不同请求，类似于客户端实现所谓的高并发
  >
  > 具体实现可以给每个请求添加一个 sequence number

- 操作系统中进程线程区别，协程了解吗

- 虚拟地址如何转换成物理地址，页表结构说一下

- 算法题：经典的反转链表以及重组链表

- 思维题：64 匹马 8 个赛道如何决出前4名

  > 首先 8 场比赛可以淘汰每组比赛后四名，剩余 32 匹马
  >
  > 其次组织上次比赛 8 个第一名进行 1 场比赛，最终可以得到总的第一名 A1
  >
  > 剩余 9 匹马 A2-A4、B1-B3、C1-C2、D1
  >
  > 不让 B1 比赛，剩余 8 匹再次赛一个，如果 B2、B3、C1、C2、D1 有一个处于前 3，可以将第 3 名剔除，A1、B1 和前 2 名就是前四（至此最少 10 场比赛可以决出前四名）
  >
  > 否则 A2、A3、A4 就是前 3，A4 和 B1 再次比一下就决出第四名（至此 11 场比赛可以决出前四名）

> 反问具体业务：推荐架构 C++、Python 语法糖、Java 上层业务
